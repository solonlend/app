...
export function FarmProtocolPanel() {
  ...
  const params = [
    ...
    ["Liquidation bonus", bps(protocol.liqBonusBps)],
    ["Protocol share", bps(protocol.protocolFeeBps)],
    [
      "Price feed timeout",
      protocol.feeds
    ...
    ["Depeg band", depegLimit === undefined ? "—" : `±${depegLimit}%`],
    // No max-leverage getter exists in UniV3DualVault: existing SOLON_FARMS display/input cap.
    ["Max leverage (UI cap)", `${SOLON_FARMS[0].maxLeverage}x`],
  ];
  return (
    <section
      aria-label="Protocol status and limits"
    ...
    >
      <div className="bg-primary overflow-hidden rounded-xl border border-white/10 text-xs">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 border-b border-white/10 px-4 py-3">
          <span className="text-farm-warning bg-farm-warning-subtle rounded-md px-2.5 py-1">SOFT LAUNCH</span>
          {protocol.feeds.map((feed, i) => {
            ...
            return (
              <span key={i} className="text-secondary-foreground">
                {feed.symbol} price feed · {known ? `${duration(age)} ago · ${stale ? "stale" : "fresh"}` : "unknown"}
              </span>
            );
          })}
          <span className="text-secondary-foreground">
            {stable.symbol} peg {stable.price === undefined ? "—" : `$${stable.price.toFixed(4)}`} · deviation{" "}
            ...
            <div key={i} className="min-w-0 overflow-hidden">
              ...
              {!reserve.unlimited && (
                <>
                  <div
                    role="progressbar"
                    aria-label={`${reserve.symbol} reserve capacity`}
                    aria-valuemin={0}
                    aria-valuemax={100}
